...
Requires(post): kernel = 4.13.0-2.rc3.gitec0d270%{dist}
Requires: %{name}-container = %{version}-%{release}
Requires(post): container-selinux = 2:2.17-2%{dist}
Requires(post): docker = 2:1.12.6-7.gitae7d637%{dist}
Requires(post): docker-swarm = 1.1.0-2.gita0fd82b
Requires(post): flannel = 0.5.5-2.gitcb8284f%{dist}
Requires(post): kubernetes = 1.2.0-0.22.git4a3f9c5%{dist}
Requires(post): skopeo = 0.1.20-2.gite802625%{dist}
Requires: %{name}-virt = %{version}-%{release}
Requires(post): SLOF = 20170303-4.gitc39657a%{dist}
Requires(post): libvirt = 3.6.0-1.git40c1264%{dist}
Requires(post): qemu = 15:2.9.0-6.git4cfb657%{dist}
Requires: %{name}-virt-management = %{version}-%{release}
Requires(post): novnc = 0.5.1-6.gitfc00821%{dist}
Requires(post): ginger = 2.3.0-18.gite9b8a1b%{dist}
Requires(post): ginger-base = 2.2.1-14.git109815c%{dist}
Requires(post): kimchi = 2.3.0-18.git3830c25%{dist}
Requires(post): wok = 2.3.0-16.git7f5e0ae%{dist}
Requires: %{name}-ras = %{version}-%{release}
Requires(post): crash = 7.1.6-2.git64531dc%{dist}
Requires(post): hwdata = 0.288-2.git625a119%{dist}
Requires(post): libnl3 = 3.2.28-5%{dist}
Requires(post): librtas = 1.4.1-3.git3fe4911%{dist}
Requires(post): libservicelog = 1.1.16-3.git48875ee%{dist}
Requires(post): libvpd = 2.2.5-5.git8cb3fe0%{dist}
Requires(post): lshw = B.02.18-2.gitf9bdcc3
Requires(post): lsvpd = 1.7.7-7.git3a5f5e1%{dist}
Requires(post): ppc64-diag = 2.7.2-2.gitd56f7f1%{dist}
Requires(post): servicelog = 1.1.14-5.git7d33cd3%{dist}
Requires(post): sos = 3.3-19.git52dd1db%{dist}
Requires(post): systemtap = 3.1-4.git39b62b4%{dist}

Requires(post): gcc = 4.8.5-14.svn240558%{dist}
Requires(post): golang-github-russross-blackfriday = 1:1.2-7.git5f33e7b%{dist}
Requires(post): golang-github-shurcooL-sanitized_anchor_name = 1:0-2.git1dba4b3%{dist}
Requires(post): golang = 1.7.1-4%{dist}
...
%changelog
* Fri Aug 11 2017 Builds Bot <bot@email.com> - 3.0-3.alpha
